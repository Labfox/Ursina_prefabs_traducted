...
class FileBrowser(Entity):
    def __init__(self, **kwargs):
        ...
        self.title_bar = Button(parent=self, scale=(.9,.035), text='<gray>Ouvrir', color=color.dark_gray, collision=False)
        ...
        self.cancel_button = Button(parent=self, scale=(.875*.24, .05), y=(-self.max_buttons*.025)-.15, origin_x=-.5, x=-.875/2, text='Annuler', on_click=self.close)
        self.open_button = Button(parent=self, scale=(.875*.74, .05), y=(-self.max_buttons*.025)-.15, origin_x=.5, x=.875/2, text='Ouvrir', color=color.dark_gray, on_click=self.open)
