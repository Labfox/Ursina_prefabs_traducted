...
class HotReloader(Entity):
    ...
    def toggle_hotreloading(self):
        ...
        print_on_screen(f'<azure>Rechargement: {self.hotreload}')



    def reload_code(self, reset_camera=True):
        if not self.path.exists:
            print('essai de rechargement, mais le fichier est introuvable:', self.path)
            return
    ...
    def reload_textures(self):
        ...
        for e in textured_entities:
            ...
            if e.texture.path.parent.name == application.compressed_textures_folder.name:
                print('La texture est faite en .psd: ', e.texture.path.stem + '.psd')
                texture_importer.compress_textures(e.texture.path.stem)
            print('Texture rechargée:', e.texture.path)
        ...
        for e in entities:
            if e.model:
                ...
                if name in changed_models:
                    ...
                    e.model = name
                    e.origin = e.origin
                    print('Model rechargé:', name, e.model)
    ...
    def reload_shaders(self):
        ...
        for shader in ursina.shader.imported_shaders.values():
            ...
            with open(shader.path, encoding='utf8') as f:
                try:
                    print('éssai de rechargement:', shader.path.name)
                    text = f.read()
                    ...
                    print('Shader rechargé:', shader.path.name)
                except Exception as e:
                    print('érreur lors du chargement du shader:', shader.path.name, 'érreur:', e)
                    pass
