...
class Conversation(Entity):
    ...
    def __init__(self, **kwargs):
        ...
        self.answer_0 = Button(parent=self, text='réponse_0', y=self.question.y-self.spacing-.025, scale=(1,.075), text_origin=(-.5,0), model=copy(self.button_model))
        self.answer_1 = Button(parent=self, text='réponse_1', y=self.answer_0.y-self.spacing, scale=(1,.075), text_origin=(-.5,0), model=copy(self.button_model))
        self.answer_2 = Button(parent=self, text='réponse_2', y=self.answer_1.y-self.spacing, scale=(1,.075), text_origin=(-.5,0), model=copy(self.button_model))
        ...
        for i, child in enumerate(answers):
            ...
            def on_click(node=child):
                ...
                if len(node.children) > 1:
                    print('érreur sur la question:', n, '. La question à plusieurs possibilités')
